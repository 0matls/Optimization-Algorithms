...
%   1st assignement 18-11-21 ex3 - Fibonacci - main script - uses 3 sup
%   functions
% 
...
for i = 1 : length(l)
    for j = 1 : length(functions)
        [a, b, k] = fibo(l(i),alpha_start,beta_start,functions{j});
        lArray(i,j) = k;
        alpha_array{i,j} = a;
        beta_array{i,j} = b;
    end
...
for i = 1 : length(functions)
    for j = 1 : num_of_diff_l_values
        ...
        title(['f' num2str(i) ' (x) and l = ' num2str(l(randj(j)))])
        xlabel('k');
